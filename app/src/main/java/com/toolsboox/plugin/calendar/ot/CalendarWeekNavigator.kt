...
class CalendarWeekNavigator {
    ...
    companion object {
            ...
            when (motionEvent.action) {
                MotionEvent.ACTION_UP -> {
                    ...
                    if (px >= lo + 0 * cew && px <= lo + 1 * cew && py >= to && py <= to + ceh) {
                        CalendarNavigator.toWeek(fragment, localDate.minusWeeks(1L), locale)
                        return true
                    }
                    ...
                    if (px >= lo + 19 * cew && px <= lo + 20 * cew && py >= to && py <= to + ceh) {
                        CalendarNavigator.toWeek(fragment, localDate.plusWeeks(1L), locale)
                        return true
                    }
                }
            }
        ...
        fun draw(context: Context, canvas: Canvas, calendarWeek: CalendarWeek) {
            ...
            val week = localDate.plusWeeks(0L).get(WeekFields.of(locale).weekOfWeekBasedYear())
